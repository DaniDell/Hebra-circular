...
const ReceptionMeasure = () => {
  ...
  const handleCalculate = () => {
    ...
    const managedPolyesterBaseKg = parseFloat(
      watch("managedPolyesterBaseKg") || 0
    );
    ...
    [
      ...
      const composition =
        inputName === "managedCottonBaseKg"
          ? "algodon Reciclado"
          : inputName === "managedPolyesterBaseKg"
          ? "poliester Reciclado"
          : inputName === "managedMixBaseKg"
          ? "Mezcla sin definición Reciclado"
          : "";
      const impact = calculateImpact(kilograms, composition);
      console.log(
        `Kilograms: ${kilograms}, Composition: ${composition}, Impact:`,
        impact
      );
      totalWaterImpact +=
        impact.waterImpactLandfill - impact.waterImpact2dnChance;
      totalCarbonImpact +=
        impact.carbonImpactLandfill - impact.carbonImpact2dnChance;
    });
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setErrorMessage(
        "Error al enviar el registro. Inténtelo de nuevo más tarde."
      );
    }
  };

  const waterImpactShow =
    totalImpact?.totalWaterImpact?.toLocaleString("de-DE") ?? "";
  const carbonImpactShow =
    totalImpact?.totalCarbonImpact?.toLocaleString("de-DE", {
  ...
  return (
    ...
    >
      ...
      >
        {" "}
        {/* Agrega onChange para detectar cambios en los inputs */}
        ...
        <TextField
          ...
          helperText={
            errors.managedCottonBaseKg ? "Ingrese un valor numérico" : ""
          }
          style={{ marginBottom: "20px" }}
        ...
        />
        <TextField
          {...register("managedPolyesterBaseKg", {
            required: true,
            pattern: /^[0-9]*$/,
          })}
          ...
          helperText={
            errors.managedPolyesterBaseKg ? "Ingrese un valor numérico" : ""
          }
          style={{ marginBottom: "20px" }}
          InputProps={{ inputProps: { min: 0 } }}
          defaultValue={0}
        ...
        <TextField
          ...
          helperText={
            errors.managedMixBaseKg ? "Ingrese un valor numérico" : ""
          }
          style={{ marginBottom: "20px" }}
          InputProps={{ inputProps: { min: 0 } }}
          defaultValue={0}
      ...
      <Dialog open={open} PaperProps={{ style: { borderRadius: 25 } }}>
        <DialogTitle>La mitigación proyectada es:</DialogTitle>
        <DialogContent>
